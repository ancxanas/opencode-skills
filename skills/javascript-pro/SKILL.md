...
metadata:
  ...
  version: "1.1.0"
  domain: language
  triggers: JavaScript, ES2025, async await, Node.js, vanilla JavaScript, Web Workers, Fetch API, browser API, module system
  role: specialist
...
|-------|-----------|-----------|
| Modern Syntax | `references/modern-syntax.md` | ES2025+ features, optional chaining, private fields, Iterator helpers, Set methods |
...
- Use Iterator helpers (`.map()`, `.filter()`, `.take()` on iterables)
- Use Set methods (`.union()`, `.intersection()`, `.difference()`)
- Use JSON modules (`import data from './data.json' assert { type: 'json' }` → `with { type: 'json' }`)
- Use `RegExp.escape()` for safe regex construction
- Use RegExp pattern modifiers (`/foo/m` flag syntax for inline mode changes)
